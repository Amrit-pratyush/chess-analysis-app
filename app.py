...
def classify_move_multipv(board_before: chess.Board, move: chess.Move, best_move: chess.Move, 
    ...
    if move == best_move and is_sac:
        if is_mate_eval and moved_piece and moved_piece.piece_type in [chess.QUEEN, chess.ROOK]:
            return "Brilliant (!!)"
        if win_before_mover <= 88.0 and (win_after_mover >= 55.0 or is_mate_eval):
            return "Brilliant (!!)"

    if move == best_move and line2_win_gap >= 12.0 and win_loss <= 0.05 and win_before_mover <= 60.0:
        return "Great (!)"

    if move == best_move or win_loss <= 0.2:
        return "Best"

    if prev_opponent_loss >= 10.0 and win_loss >= 12.0:
        return "Miss"
    if win_before_mover >= 75.0 and win_loss >= 22.0:
        return "Miss"
